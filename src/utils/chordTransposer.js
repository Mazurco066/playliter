...
/*!
 * Vue js chord transposer based on https://github.com/jessegavin/jQuery-Chord-Transposer
 */
(function($) {
  ...
  $.fn.transpose = function(options) {
    ...
    const transposeSong = function (target, key) {
      ...
      $('span.c', target).each(function (_, el) {
    ...
    const transposeChord = function (selector, delta, targetKey) {
      var el = $(selector)
    ...
    const getNewWhiteSpaceLength = function (a, b, c) {
      if (a > b) return (c + (a - b))
      else if (a < b) return (c - (b - a))
      else return c
    };

    const makeString = function (s, repeat) {
      var o = []
    ...
    const isChordLine = function (input) {
      ...
      for (var i = 0; i < tokens.length; i++) {
        if (!$.trim(tokens[i]).length === 0 && !tokens[i].match(opts.chordRegex)) return false
      }
      return true
    }
